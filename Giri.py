...
#        global Giri_cutter
        ...
        img_tk = ImageTk.PhotoImage(img_resized, master=self.tk)
        ...
        f_data_list = [{'name': "back_one" , 'command': self.back_one, 'text': "一つ前に戻る",},
            {'name': "trim_fin", 'command': self.trim_fin, 'text': "入力完了\n(保存して戻る)"},
        ...
        canvas1.bind("<ButtonPress-1>", self.start_point_get)
        canvas1.bind("<Button1-Motion>", self.rect_drawing)
        canvas1.bind("<ButtonRelease-1>", self.release_action)
    ...
    def release_action(self, event):
        ...
        if qCnt == 0:
            ...
            # canvas1上に四角形を描画（rectangleは矩形の意味）
            self.create_rectangle_alpha(pos[0], pos[1], pos[2], pos[3],
                               fill="green",
                               alpha=0.3,
                               tag="nameBox"
                               )
    ...
    def create_rectangle_alpha(self, x1, y1, x2, y2, **kwargs):
        if 'alpha' in kwargs:
            ...
            fill = self.tk.winfo_rgb(fill) + (alpha,)
            image = Image.new('RGBA', (x2-x1, y2-y1), fill)
            self.images.append(ImageTk.PhotoImage(image, master=self.tk))
            canvas1.create_image(x1, y1, image=self.images[-1], anchor='nw')
